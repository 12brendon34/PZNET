...
static unsigned g_ipv4SocketIndex = 0;
static bool g_mapIPv4ToV6 = false;
...
extern "C" {
    ...
    JNIEXPORT jint JNICALL Java_zombie_core_raknet_RakNetPeerInterface_Connect(JNIEnv* env, jobject, jstring host, jint port, jstring password, jboolean /*useSteam*/)
    {
        ...
        std::string connectHost = hostStr;
        unsigned connSocketIndex = 0;

        in_addr v4{};
        if (inet_pton(AF_INET, hostStr, &v4) == 1) {
            connSocketIndex = g_ipv4SocketIndex;
            if (g_mapIPv4ToV6) {
                connectHost = "::ffff:" + connectHost;
                ZNetLogPrintf(1, "Connect: mapped '%s' -> '%s'\n", hostStr, connectHost.c_str());
            }
        }

        ZNetLogPrintf(2, "Connecting to %s:%d (socket %u)\n", connectHost.c_str(), port, connSocketIndex);

        const jint result = g_peer->Connect(connectHost.c_str(), static_cast<unsigned short>(port), pwStr, static_cast<int>(std::strlen(pwStr)), nullptr, connSocketIndex, 12, 500, 0);
